...
class MetaTags implements Arrayable
{
     ...
     * @return array
     */
    private function replacePlaceholders($tag)
    {
        ...
        if (array_key_exists('content', $tag)) {
            $text = $replacer->execute($tag['content'], $this->data);
            if ( ! $text) return null;
            $tag['content'] = $shouldEscapeQuotes ? str_replace('"', '&quot', $text) : $text;
        } else if (array_key_exists('_text', $tag)) {
            $text = $replacer->execute($tag['_text'], $this->data);
            if ( ! $text) return null;
            $tag['_text'] = $shouldEscapeQuotes ? str_replace('"', '&quot', $text) : $text;
        }
